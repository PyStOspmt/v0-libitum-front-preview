...
import { ArrowUpRight, CheckCircle2, Coins, Gem, Gift, Star, TrendingUp } from "lucide-react"
                        ...
                        <div className="lg:col-span-2 space-y-6">
                            ...
                            <div className="grid gap-4 md:grid-cols-2">
                                {rewards.map((reward) => (
                                    <Card
                                        key={reward.id}
                                        className="rounded-[20px] border border-slate-200/80 shadow-[0_4px_12px_rgba(0,0,0,0.03)] overflow-hidden flex flex-col"
                                    >
                                        <CardHeader className="pb-4">
                                            <div className="flex justify-between items-start mb-2">
                                                <div
                                                    className={`h-12 w-12 rounded-[12px] flex items-center justify-center ${reward.color}`}
                                                >
                                                    {reward.icon}
                                                </div>
                                                <Badge
                                                    variant="outline"
                                                    className="bg-[#f0f3f3] border-0 text-[#121117] font-[600] px-3 py-1 text-[14px]"
                                                >
                                                    {reward.price} LC
                                                </Badge>
                                            </div>
                                        ...
                                        <CardContent className="flex-1">
                                            <p className="text-[14px] text-[#69686f] leading-relaxed">{reward.description}</p>
                                        </CardContent>
                                    ...
                                    {history.map((item) => (
                                        ...
                                        >
                                            <div>
                                                ...
                                                <p className="text-[12px] text-[#69686f] mt-0.5">
                                                    {new Date(item.date).toLocaleDateString("uk-UA")}
                                                </p>
                                            </div>
                                            <span
                                                className={`text-[14px] font-[700] ${item.type === "earn" ? "text-[var(--theme-primary)]" : "text-[#121117]"}`}
                                            >
                                                {item.amount}
                                            </span>
                                        </div>
                        ...
                        <DialogContent className="max-w-md rounded-[24px] p-8 font-sans">
                            ...
                            <div className="py-6">
                                ...
                                <div className="flex items-center justify-between p-4 border-t border-slate-200 mt-2">
                                    ...
                                    <span className="text-[18px] font-bold text-[var(--theme-primary)]">
                                        {lcBalance - (selectedReward?.price || 0)} LC
                                    </span>
                                </div>
                            </div>
