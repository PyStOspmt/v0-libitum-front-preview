...
import { Award, CheckCircle2, Clock, Star, Target, TrendingUp, Zap } from "lucide-react"
                            ...
                            <div className="space-y-6">
                                <div>
                                    <div className="mb-2 flex items-center justify-between">
                                        <span className="text-[14px] font-[600] text-[#121117]">
                                            Прогрес до наступного рівня
                                        </span>
                                        <span className="font-[600] text-[var(--theme-primary)]">
                                            {Math.round(categoryProgress.percentage)}%
                                        </span>
                                    </div>
                                    <Progress
                                        value={categoryProgress.percentage}
                                        className="h-3 bg-gray-100 [&>div]:bg-[var(--theme-primary)]"
                                    />
                                </div>
                                    ...
                                    <div className="flex flex-wrap gap-2">
                                        {currentLevel.benefits.map((benefit) => (
                                            <Badge
                                                key={benefit}
                                                variant="outline"
                                                className="bg-[var(--theme-primary-light)] text-[var(--theme-primary-dark)] border-0 px-3 py-1.5 text-[14px] font-[500] rounded-[8px]"
                                            >
                    ...
                    <div className="grid gap-4 sm:gap-6 grid-cols-1 sm:grid-cols-2 lg:grid-cols-3">
                        ...
                        <div className="bg-white rounded-[24px] p-6 border border-slate-200/80 shadow-[0_15px_35px_rgba(0,0,0,0.08)]">
                            ...
                            <Progress
                                ...
                                className="mt-4 h-2 bg-gray-100 [&>div]:bg-[var(--theme-primary)]"
                            />
                        </div>
                        ...
                        <div className="grid gap-4 sm:gap-6 grid-cols-1 sm:grid-cols-2 lg:grid-cols-3">
                            {availableAchievements.map((achievement) => {
                                ...
                                return (
                                    <div
                                        key={achievement.id}
                                        className={`rounded-[16px] border p-5 transition-all ${unlocked ? "border-[var(--theme-primary)]/30 bg-[var(--theme-primary-light)]" : "border-slate-200/80 bg-white opacity-60 grayscale-[0.5]"}`}
                                    >
                                        <div className="flex items-start gap-4">
                                            <div className="text-[32px] shrink-0 leading-none">{achievement.icon}</div>
                                            <div className="flex-1">
                                                <p
                                                    className={`font-[600] text-[16px] ${unlocked ? "text-[#121117]" : "text-[#69686f]"}`}
                                                >
                                                    {achievement.title}
                                                </p>
                                                <p
                                                    className={`text-[14px] mt-1 ${unlocked ? "text-[#121117]/80" : "text-[#69686f]"}`}
                                                >
                                                ...
                                                {unlocked && (
                                                    <p className="mt-2 text-[12px] font-[600] text-[var(--theme-primary-dark)]">
                                                        Розблоковано{" "}
                                                        {new Date(unlocked.unlockedAt!).toLocaleDateString("uk-UA")}
                                                    </p>
                                                )}
                                            </div>
                                            {unlocked && (
                                    ...
                                    <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-4">
                                        <div className="flex-1">
                                            <div className="mb-2 flex items-center flex-wrap gap-3">
                                                <h3
                                                    className={`font-bold text-[18px] ${level.level === currentLevel.level ? "text-[#121117]" : "text-[#121117]"}`}
                                                >
                                                    Рівень {level.level}: {level.title}
                                                </h3>
                                                {level.level === currentLevel.level && (
                                                    <Badge className="bg-[#121117] text-white border-0 hover:bg-[#121117]">
                                                        Поточний
                                                    </Badge>
                                                )}
                                                {level.level < currentLevel.level && (
                                                    <Badge
                                                        variant="outline"
                                                        className="bg-[#f0f3f3] text-[#69686f] border-0 px-3 py-1 rounded-[6px]"
                                                    >
                                            ...
                                            <p
                                                className={`mb-4 text-[15px] font-[500] ${level.level === currentLevel.level ? "text-[var(--theme-primary-dark)]" : "text-[#69686f]"}`}
                                            >
                                                {level.minXP === 0
                                                    ? `0 - ${level.maxXP} XP`
                                            ...
                                            <div className="flex flex-wrap gap-2">
                                                {level.benefits.map((benefit) => (
                                                    <Badge
                                                        key={benefit}
                                                        variant="outline"
                                                        className={`border-slate-200/80 px-3 py-1.5 text-[14px] font-[500] rounded-[8px] ${level.level === currentLevel.level ? "bg-white text-[#121117]" : "bg-[#f0f3f3] text-[#69686f]"}`}
                                                    >
                                                        {benefit}
                                                    </Badge>
                                                ))}
