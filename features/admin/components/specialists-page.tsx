...
import { Ban, CheckCircle2, Eye, LogIn, Mail, Phone, Search, XCircle } from "lucide-react"
                ...
                <div className="container mx-auto max-w-7xl space-y-6 p-6 font-sans">
                    ...
                    <Tabs defaultValue="all" className="w-full">
                        <TabsList className="bg-[#f0f3f3] rounded-[12px] p-1 border-0">
                            ...
                            >
                                Всі ({filteredSpecialists.length})
                            </TabsTrigger>
                            <TabsTrigger
                                value="active"
                                className="rounded-[8px] data-[state=active]:bg-white data-[state=active]:text-[#121117] data-[state=active]:shadow-sm font-[600] text-[#69686f]"
                            >
                                Активні ({activeSpecialists.length})
                            </TabsTrigger>
                            <TabsTrigger
                                value="pending"
                                className="rounded-[8px] data-[state=active]:bg-white data-[state=active]:text-[#121117] data-[state=active]:shadow-sm font-[600] text-[#69686f]"
                            >
                                Очікують ({pendingSpecialists.length})
                            </TabsTrigger>
                            <TabsTrigger
                                value="blocked"
                                className="rounded-[8px] data-[state=active]:bg-white data-[state=active]:text-[#121117] data-[state=active]:shadow-sm font-[600] text-[#69686f]"
                            >
                                Заблоковані ({blockedSpecialists.length})
                            </TabsTrigger>
                        </TabsList>
                    ...
                    <div className="flex-1 space-y-3 min-w-0">
                        ...
                        <div className="flex flex-wrap gap-2">
                            {specialist.subjects.map((subject: string) => (
                                <Badge
                                    key={subject}
                                    variant="outline"
                                    className="rounded-[8px] bg-white text-[#121117] border-slate-200/80 font-[500] px-3 py-1"
                                >
                                    {subject}
                                </Badge>
                            ))}
                        ...
                        <div className="flex gap-4 text-[14px] font-[600] text-[#121117] flex-wrap bg-[#f0f3f3]/50 p-3 rounded-[12px] border border-slate-200/80">
                            <span className="flex items-center gap-1.5">
                                <span className="text-[#69686f] font-[500]">Рейтинг:</span>{" "}
                                {specialist.rating > 0 ? specialist.rating : "Немає"}
                            </span>
                            <span className="flex items-center gap-1.5">
                                <span className="text-[#69686f] font-[500]">Занять:</span> {specialist.totalLessons}
                            </span>
                            <span className="flex items-center gap-1.5">
                                <span className="text-[#69686f] font-[500]">Ціна:</span> {specialist.price}
                            </span>
                            <span className="flex items-center gap-1.5">
                                <span className="text-[#69686f] font-[500]">Приєднався:</span>{" "}
                                {new Date(specialist.joinedDate).toLocaleDateString("uk-UA")}
                            </span>
                        </div>
                    </div>
                    ...
                    <div className="flex flex-col sm:flex-row flex-wrap gap-2 w-full shrink-0 md:w-auto">
                            ...
                            <DialogContent className="rounded-[24px] border-0 shadow-[0_8px_32px_rgba(0,0,0,0.08)] sm:max-w-xl font-sans">
                                <DialogHeader className="pb-4 border-b border-slate-200/80">
                                    ...
                                    <DialogDescription className="text-[#69686f] font-[500] text-[15px]">
                                        Повна інформація про {specialist.name}
                                    </DialogDescription>
                                </DialogHeader>
                                <div className="space-y-4">
                                    ...
                                    <div>
                                        ...
                                        <p className="text-[16px] font-[600] text-[#121117] mt-1">
                                    ...
                                    <div>
                                        ...
                                        <p className="text-[15px] font-[500] text-[#121117] bg-[#f0f3f3]/50 p-3 rounded-[12px] mt-1">
                                            Ціна: {specialist.price} • Рейтинг: {specialist.rating || "Немає"} • Занять:{" "}
                                            {specialist.totalLessons}
                                        </p>
                                    </div>
                                    <div>
                        ...
                        {specialist.status === "pending" && (
                            <div className="flex flex-col sm:flex-row gap-2 w-full sm:w-auto">
                                <AlertDialog
                                    ...
                                    onOpenChange={(open) => setConfirmAction(open ? "approve" : null)}
                                >
                                ...
                                <Dialog>
                                    <DialogTrigger asChild>
                                        <Button
                                            size="sm"
                                            variant="destructive"
                                            className="w-full sm:w-auto rounded-[8px] font-[600]"
                                        >
                        ...
                        {specialist.status !== "blocked" ? (
                            <AlertDialog
                                ...
                                onOpenChange={(open) => setConfirmAction(open ? "block" : null)}
                            >
                                <AlertDialogTrigger asChild>
                                    <Button
                                        size="sm"
                                        variant="outline"
                                        className="w-full sm:w-auto rounded-[8px] border-red-200 text-red-600 hover:bg-red-50 font-[600]"
                                    >
                                ...
                                <AlertDialogContent className="rounded-[24px] border-0 shadow-[0_8px_32px_rgba(0,0,0,0.08)] font-sans">
                                    <AlertDialogHeader>
                                        <AlertDialogTitle className="text-[20px] font-[700] text-[#121117]">
                                            Заблокувати спеціаліста?
                                        </AlertDialogTitle>
                                        <AlertDialogDescription className="text-[#69686f] font-[500]">
                                            Він втратить доступ до кабінету до розблокування.
                                        </AlertDialogDescription>
                                    </AlertDialogHeader>
                                    <AlertDialogFooter>
                                        <AlertDialogCancel className="rounded-[8px] border-slate-200/80 hover:bg-[#f0f3f3] text-[#121117] font-[600]">
                                        ...
                                        <AlertDialogAction
                                            onClick={() => setConfirmAction(null)}
                                            className="rounded-[8px] bg-red-600 text-white hover:bg-red-700 font-[600]"
                                        >
                                            Заблокувати
                                        </AlertDialogAction>
                                    </AlertDialogFooter>
                            ...
                            <AlertDialog
                                ...
                                onOpenChange={(open) => setConfirmAction(open ? "unblock" : null)}
                            >
                                <AlertDialogTrigger asChild>
                                    <Button
                                        size="sm"
                                        variant="default"
                                        className="w-full sm:w-auto rounded-[8px] bg-[#121117] text-white hover:bg-[#121117]/90 font-[600]"
                                    >
                                        Розблокувати
                                    </Button>
                                </AlertDialogTrigger>
                                <AlertDialogContent className="rounded-[24px] border-0 shadow-[0_8px_32px_rgba(0,0,0,0.08)] font-sans">
                                    <AlertDialogHeader>
                                        <AlertDialogTitle className="text-[20px] font-[700] text-[#121117]">
                                            Розблокувати спеціаліста?
                                        </AlertDialogTitle>
                                        <AlertDialogDescription className="text-[#69686f] font-[500]">
                                            Доступ до кабінету буде відновлено.
                                        </AlertDialogDescription>
                                    </AlertDialogHeader>
                                    <AlertDialogFooter>
                                        <AlertDialogCancel className="rounded-[8px] border-slate-200/80 hover:bg-[#f0f3f3] text-[#121117] font-[600]">
                                        ...
                                        <AlertDialogAction
                                            onClick={() => setConfirmAction(null)}
                                            className="rounded-[8px] bg-[#121117] text-white hover:bg-[#121117]/90 font-[600]"
                                        >
                                            Розблокувати
                                        </AlertDialogAction>
                                    </AlertDialogFooter>
