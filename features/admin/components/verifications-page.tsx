        ...
        <div className="container mx-auto max-w-7xl space-y-6 p-6 font-sans">
          ...
          <Card className="rounded-[24px] border-slate-200/80 shadow-[0_4px_24px_rgba(0,0,0,0.04)]">
            <CardHeader>
              ...
              <CardDescription className="text-[#69686f] font-[500]">{pendingVerifications.length} спеціалістів очікують перевірки</CardDescription>
            </CardHeader>
            <CardContent className="space-y-4">
              {pendingVerifications.map((verification) => (
                <Card key={verification.id} className="rounded-[16px] border-slate-200/80 shadow-none hover:shadow-[0_4px_12px_rgba(0,0,0,0.05)] transition-shadow">
                  <CardContent className="p-6">
                    <div className="space-y-5">
                      <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-3">
                        <div className="space-y-2">
                          <h3 className="text-lg font-[700] text-[#121117]">{verification.specialistName}</h3>
                          <p className="text-[14px] font-[500] text-[#69686f]">{verification.email}</p>
                          <Badge className="rounded-[8px] bg-[#f0f3f3] text-[#121117] hover:bg-[#f0f3f3]/80 border-0 font-[600]">{verification.specialization}</Badge>
                        </div>
                      ...
                      <div className="grid gap-4 md:grid-cols-2 bg-[#f0f3f3]/30 p-4 rounded-[12px] border border-slate-200/50">
                        <div>
                          <Label className="text-[#69686f] font-[600] text-[13px] uppercase tracking-wide">Предмети</Label>
                          <div className="mt-2 flex flex-wrap gap-2">
                            {verification.subjects.map((subject) => (
                              <Badge key={subject} variant="outline" className="rounded-[8px] bg-white text-[#121117] border-slate-200/80 font-[500]">
                                {subject}
                              </Badge>
                            ))}
                          </div>
                        ...
                        <div>
                          <Label className="text-[#69686f] font-[600] text-[13px] uppercase tracking-wide">Досвід</Label>
                          <p className="mt-2 text-[15px] font-[500] text-[#121117]">{verification.experience}</p>
                        </div>
                      </div>

                      <div className="bg-[#f0f3f3]/30 p-4 rounded-[12px] border border-slate-200/50">
                        <Label className="text-[#69686f] font-[600] text-[13px] uppercase tracking-wide">Освіта</Label>
                        <p className="mt-2 text-[15px] font-[500] text-[#121117]">{verification.education}</p>
                      </div>

                      <div className="bg-[#f0f3f3]/30 p-4 rounded-[12px] border border-slate-200/50">
                        <Label className="text-[#69686f] font-[600] text-[13px] uppercase tracking-wide">Про себе</Label>
                        <p className="mt-2 text-[15px] font-[500] text-[#121117] leading-relaxed">{verification.description}</p>
                      </div>

                      <div className="bg-[#f0f3f3]/30 p-4 rounded-[12px] border border-slate-200/50">
                        <Label className="text-[#69686f] font-[600] text-[13px] uppercase tracking-wide">Документи</Label>
                        <div className="mt-2 flex flex-wrap gap-2">
                          {verification.documents.map((doc) => (
                            <Button key={doc} variant="outline" size="sm" className="rounded-[8px] border-slate-200/80 text-[#121117] font-[600] hover:bg-white shadow-sm">
                      ...
                      <div className="flex flex-col sm:flex-row gap-3 pt-2">
                        ...
                        <Dialog>
                          <DialogTrigger asChild>
                            <Button variant="destructive" className="flex-1 rounded-[8px] font-[600]">
                          ...
                          <DialogContent className="rounded-[24px] border-0 shadow-[0_8px_32px_rgba(0,0,0,0.08)] font-sans sm:max-w-md">
                            <DialogHeader className="pb-4 border-b border-slate-200/80">
                              <DialogTitle className="text-[20px] font-[700] text-[#121117]">Відхилити верифікацію</DialogTitle>
                              <DialogDescription className="text-[#69686f] font-[500]">
                                Вкажіть причину відхилення для <span className="font-[600] text-[#121117]">{verification.specialistName}</span>
                              </DialogDescription>
                            </DialogHeader>
                            <div className="space-y-4 py-4">
                              <div className="space-y-2">
                                <Label className="text-[#121117] font-[600]">Причина відхилення</Label>
                                <Textarea
                            ...
                            <DialogFooter className="pt-2">
                              <Button variant="outline" className="rounded-[8px] border-slate-200/80 hover:bg-[#f0f3f3] text-[#121117] font-[600]">Скасувати</Button>
                              <Button variant="destructive" className="rounded-[8px] font-[600]">Відхилити</Button>
                            </DialogFooter>
                          </DialogContent>
                        </Dialog>
                      </div>
