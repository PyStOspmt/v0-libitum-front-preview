...
const specialists = [
    {
        ...
        bioText:
            "Маю 5 років досвіду викладання англійської мови. Працюю з учнями різного віку — від школярів до дорослих. Адаптую програму під цілі кожного учня.",
    ...
    },
    {
        ...
        bioText:
            "Клінічний психолог з 10-річним досвідом. Спеціалізуюсь на роботі з тривожністю, депресією та стресом. Проводжу індивідуальну та сімейну терапію.",
    ...
    },
    {
        ...
        bioText:
            "Працюю з дітьми та дорослими. Корекція вимови, постановка звуків, розвиток мовлення. Індивідуальний підхід до кожного клієнта.",
        popular: "",
    },
]
...
function PillDropdown({
    ...
    const hasValue = multi ? (value as string[]).length > 0 : value !== "all"
                    ...
                    {multi
                        ? options.map((opt) => {
                              ...
                              return (
                                  ...
                                  >
                                      <div
                                          className={`w-4 h-4 rounded border flex items-center justify-center text-[10px] ${selected ? "bg-slate-900 border-slate-900 text-white" : "border-slate-300"}`}
                                      >
                                          {selected && "✓"}
                                      </div>
                                      {opt.label}
...
/* ═══════════════════════════════════════════════════
   Main Page
   ═══════════════════════════════════════════════════ */
export function SpecialistsCatalogPage() {
    const { user } = useAuth()
    ...
    const [category, setCategory] = useState(
        categoryParam && ["tutor", "psychologist", "speech-therapist"].includes(categoryParam) ? categoryParam : "all",
    )
    ...
    const filtered = specialists
        ...
        .sort((a, b) => {
            ...
            switch (sortBy) {
                case "price-asc":
                    return a.pricePerLesson - b.pricePerLesson
                case "price-desc":
                    return b.pricePerLesson - a.pricePerLesson
                case "rating":
    ...
    return (
        <div className="min-h-screen bg-gradient-to-br from-slate-50 via-white to-white overflow-x-clip">
            ...
            />
            <div
                className="hidden sm:block fixed bottom-40 left-1/4 w-80 h-80 rounded-full bg-emerald-50/30 blur-3xl animate-orb pointer-events-none"
            ...
            <div className="max-w-[1200px] mx-auto px-4 sm:px-6">
                ...
                <div className="pt-4 pb-2">
                    ...
                    <p className="text-[15px] text-slate-600 mt-1 max-w-2xl leading-relaxed">
                        {t("catalog.page_subtitle_start") || "Оберіть з"} {filtered.length}{" "}
                        {t("catalog.page_subtitle_end") ||
                            "перевірених спеціалістів. Запишіться на пробне заняття та почніть навчання вже сьогодні."}
                    </p>
                </div>

                {/* ── Sticky Filters Container ── */}
                <div
                    className="sticky top-[72px] z-40 bg-white/80 backdrop-blur-md pt-3 pb-2 -mx-4 sm:-mx-6 px-4 sm:px-6 border-b border-transparent [&.stuck]:border-slate-100/50 [&.stuck]:shadow-sm [&.stuck]:bg-white/90"
                ...
                >
                                ...
                                <Select value={category} onValueChange={setCategory}>
                                    ...
                                    <SelectContent>
                                        ...
                                        <SelectItem value="speech-therapist">
                                            {t("filter.speech_therapists") || "Логопеди"}
                                        </SelectItem>
                                    </SelectContent>
                                </Select>
                            </div>
                                ...
                                <div className="grid grid-cols-2 gap-2">
                                    <div className="border border-slate-200 rounded-lg px-3 py-2.5 hover:border-slate-400 transition-colors bg-white">
                                        <div className="text-[10px] text-slate-400 leading-none mb-1">
                                            {t("filter.price") || "Ціна за заняття"}
                                        </div>
                                        <Select value={priceRange} onValueChange={setPriceRange}>
                                            ...
                                            <SelectContent>
                                                ...
                                                {PRICE_RANGES.slice(1).map((r, i) => (
                                                    <SelectItem key={i} value={`price-${i + 1}`}>
                                                        {r.label}
                                                    </SelectItem>
                                                ))}
                                            </SelectContent>
                    ...
                    <div className="hidden sm:block">
                        <div className="grid grid-cols-4 gap-3 mb-3">
                            <FilterBox
                                label={t("filter.specialization") || "Спеціалізація"}
                                onClick={() => categorySelectRef.current?.click()}
                            >
                                <Select value={category} onValueChange={setCategory}>
                                    <SelectTrigger
                            ...
                            <FilterBox
                                label={t("filter.price") || "Ціна за заняття"}
                                onClick={() => priceSelectRef.current?.click()}
                            >
                                <Select value={priceRange} onValueChange={setPriceRange}>
                                    <SelectTrigger
                                        ref={priceSelectRef}
                            ...
                            <FilterBox
                                label={t("filter.format") || "Формат занять"}
                                onClick={() => formatSelectRef.current?.click()}
                            >
                                <Select value={format} onValueChange={setFormat}>
                                    <SelectTrigger
                                        ref={formatSelectRef}
                    ...
                    {(() => {
                        ...
                        return filtered.map((specialist, index) => {
                            ...
                            return (
                                <div
                                    key={specialist.id}
                                    className="relative group/card z-10 hover:z-[30] transition-all duration-300"
                                >
                                            ...
                                            <div className="flex gap-6">
                                                    ...
                                                    <div className="relative w-[140px] h-[140px] rounded-[16px] overflow-hidden bg-slate-100">
                                                        {specialist.image ? (
                                                            ...
                                                            <Avatar className="h-full w-full rounded-[16px]">
                                                                <AvatarFallback className="bg-slate-100 text-2xl font-bold text-slate-600 rounded-[16px]">
                                                                    {specialist.name[0]}
                                                                </AvatarFallback>
                                                            </Avatar>
                                                        )}
                                                        {specialist.online && (
                                                            <div className="absolute bottom-2 left-2 flex items-center justify-center bg-white rounded-full p-0.5">
                                                                <div
                                                                    className={`w-3 h-3 rounded-full border-2 border-white ${isHealth ? "bg-[#f57c00]" : "bg-[#00c5a6]"}`}
                                                                />
                                                            </div>
                                                        )}
                                                    </div>
                                                ...
                                                <div className="flex-1 min-w-0 flex flex-col pt-1">
                                                    <div className="flex items-center justify-between mb-1.5">
                                                        <div className="flex items-center gap-3 flex-wrap">
                                                            <Link href={`/specialists/${specialist.id}`}>
                                                                <h3
                                                                    className={`text-[22px] font-bold text-[#121117] transition-colors leading-tight ${isHealth ? "hover:text-[#f57c00]" : "hover:text-[#00c5a6]"}`}
                                                                >
                                                                    {specialist.name}
                                                                </h3>
                                                            </Link>
                                                            {specialist.verified && (
                                                                <span
                                                                    className={`px-2 py-0.5 rounded-full text-[12px] font-bold flex items-center gap-1 border ${isHealth ? "bg-[#fff8e1] text-[#f57c00] border-[#ffc800]/40" : "bg-[#e8f5e9] text-[#00c5a6] border-[#a5d6a7]/50"}`}
                                                                >
                                                                    <Award
                                                                        className={`h-3.5 w-3.5 flex-shrink-0 ${isHealth ? "fill-[#f57c00] text-[#f57c00]" : "fill-[#00c5a6] text-[#00c5a6]"}`}
                                                                    />
                                                            ...
                                                            <div className="flex items-center gap-1 text-[15px] font-bold">
                                                                ...
                                                                <Link
                                                                    href={`/specialists/${specialist.id}?tab=reviews`}
                                                                    className="text-[#69686f] text-[13px] font-medium underline decoration-dashed underline-offset-2 ml-1 cursor-pointer hover:text-[#f57c00]"
                                                                >
                                                                    {specialist.reviews}{" "}
                                                                    {t("catalog.reviews_count") || "відгуків"}
                                                                </Link>
                                                            </div>
                                                            <div className="flex items-center gap-4 text-[13px] text-[#69686f] ml-auto mr-12 hidden md:flex">
                                                                <div className="flex flex-col items-center">
                                                                    <span className="font-bold text-[#121117] leading-none">
                                                                        {specialist.activeStudents}
                                                                    </span>
                                                                    <span className="text-[11px] mt-0.5">
                                                                        {t("catalog.students_label") || "учнів"}
                                                                    </span>
                                                                </div>
                                                                <div className="flex flex-col items-center">
                                                                    <span className="font-bold text-[#121117] leading-none">
                                                                    ...
                                                                    <span className="text-[11px] mt-0.5">
                                                                        {t("catalog.lessons_label") || "занять"}
                                                                    </span>
                                                                </div>
                                                            </div>
                                                        </div>
                                                    ...
                                                    <div className="flex items-center gap-2 mb-3">
                                                        <span
                                                            className={`text-[13px] font-bold px-3 py-1 rounded-full ${a.badge} border-transparent inline-flex items-center shadow-sm`}
                                                        >
                                                            {specialist.specialization === "Репетитор"
                                                                ? t("filter.tutors") || "Репетитори"
                                                                : specialist.specialization === "Психолог"
                                                                  ? t("filter.psychologists") || "Психологи"
                                                                  : t("filter.speech_therapists") || "Логопеди"}
                                                        </span>
                                                        <span className="text-[13px] font-medium bg-[#f0f3f3] text-[#4d4c53] px-3 py-1 rounded-full border border-slate-200 shadow-sm leading-none flex items-center h-[26px]">
                                                            {t("catalog.experience") || "Досвід:"}{" "}
                                                            <strong className="text-[#121117] ml-1">
                                                                {specialist.experience} {t("catalog.years") || "років"}
                                                            </strong>
                                                        </span>
                                                    </div>
                                                    ...
                                                    <div className="mb-2.5">
                                                        <SubjectsLine
                                                            ...
                                                            textClass="text-[16px] text-[#121117] font-semibold"
                                                            iconClass="text-[#121117] hidden"
                                                            maxCharsFallback={50}
                                                        />
                                                    </div>
                                                    ...
                                                    >
                                                        <p
                                                            className={`text-[16px] text-[#3e3d45] leading-relaxed font-medium ${isExpanded ? "" : "line-clamp-2"} relative`}
                                                        >
                                                            <span className="font-bold text-[#121117] mr-1.5">
                                                                {specialist.bioTitle} •
                                                            </span>
                                                ...
                                                <div className="flex flex-col flex-shrink-0 w-[240px] pl-6 border-l border-slate-100 justify-center">
                                                    <div className="bg-slate-50/50 rounded-2xl p-4 border border-slate-100/50 mb-4 flex items-center justify-center gap-1.5">
                                                        <span className="text-[32px] font-black text-[#121117] leading-none tracking-tight">
                                                            ₴{specialist.pricePerLesson}
                                                        </span>
                                                        <span className="text-[14px] font-medium text-[#69686f] mt-2">
                                                            / {specialist.lessonDuration}
                                                        </span>
                                                    </div>
                                            ...
                                            <div className="relative z-10 border border-slate-200 rounded-[24px] overflow-hidden bg-white shadow-[0_20px_40px_-20px_rgba(0,0,0,0.2)]">
                                                ...
                                                <div className="p-3 space-y-2 bg-[#f0f3f3]/50">
                                                    <Link href={`/specialists/${specialist.id}#schedule`} className="block">
                                                        <Button
                                                            variant="outline"
                                                            className="w-full h-10 rounded-[10px] text-[13px] font-bold border-slate-200 text-[#121117] bg-white hover:bg-slate-50 flex items-center justify-center gap-2"
                                                        >
                                    ...
                                    >
                                        <div className="flex gap-4 items-start mb-4">
                                            <Link href={`/specialists/${specialist.id}`} className="flex-shrink-0 relative">
                                                <div className="relative w-[96px] h-[96px] rounded-[16px] overflow-hidden bg-slate-100 ring-1 ring-slate-100">
                                                    {specialist.image ? (
                                                        ...
                                                        <Avatar className="h-full w-full rounded-[16px]">
                                                            <AvatarFallback className="bg-slate-100 text-[22px] font-bold text-slate-600 rounded-[16px]">
                                                                {specialist.name[0]}
                                                            </AvatarFallback>
                                                        </Avatar>
                                                    )}
                                                    {specialist.online && (
                                                        <div
                                                            className={`absolute bottom-1 right-1 w-3 h-3 rounded-full border-2 border-white ${isHealth ? "bg-[#f57c00]" : "bg-[#00c5a6]"}`}
                                                        />
                                                    )}
                                                </div>
                                            </Link>
                                            <div className="flex-1 min-w-0 flex flex-col pt-0.5">
                                                <div className="flex items-center justify-between mb-1.5">
                                                    <div className="flex items-center gap-1.5 min-w-0 pr-2">
                                                        <Link href={`/specialists/${specialist.id}`} className="min-w-0 block">
                                                            <h3 className="text-[18px] font-bold text-[#121117] leading-tight truncate">
                                                                {specialist.name}
                                                            </h3>
                                                        </Link>
                                                        {specialist.verified && (
                                                            <Award
                                                                className={`h-4 w-4 flex-shrink-0 ${isHealth ? "text-[#f57c00]" : "text-[#00c5a6]"}`}
                                                            />
                                                        )}
                                                    </div>
                                                    <button
                                                ...
                                                <div className="flex items-center gap-2 mb-2">
                                                    <span
                                                        className={`text-[12px] font-bold px-2.5 py-1 rounded-[8px] ${a.badge} border-transparent inline-flex items-center`}
                                                    >
                                                        {specialist.specialization === "Репетитор"
                                                            ? t("filter.tutors") || "Репетитор"
                                                            : specialist.specialization === "Психолог"
                                                              ? t("filter.psychologists") || "Психолог"
                                                              : t("filter.speech_therapists") || "Логопед"}
                                                    </span>
                                                </div>
                                                ...
                                                <div className="flex items-center gap-2 mt-auto">
                                                    ...
                                                    <Link
                                                        href={`/specialists/${specialist.id}?tab=reviews`}
                                                        className="text-[#69686f] text-[13px] font-medium underline decoration-dashed underline-offset-2 cursor-pointer hover:text-[#f57c00]"
                                                    >
                                        ...
                                        <div className="flex flex-col gap-[3px] mb-2">
                                            <SubjectsLine
                                                specialist={specialist}
                                                textClass="text-[15px] text-[#121117] font-semibold"
                                                iconClass={a.icon}
                                                maxCharsFallback={32}
                                            />
                                            ...
                                            <div className="flex items-center gap-3 text-[13px] text-[#69686f]">
                                                <div className="flex items-center gap-1">
                                                    ...
                                                    <span>
                                                        <strong className="text-[14px] text-[#121117]">
                                                            {specialist.activeStudents}
                                                        </strong>{" "}
                                                        учнів
                                                    </span>
                                        ...
                                        >
                                            <p
                                                className={`text-[14px] text-[#3e3d45] leading-relaxed ${isExpanded ? "" : "line-clamp-2"}`}
                                            >
                                        ...
                                        <div className="flex items-center justify-between gap-3 pt-4 border-t border-slate-100">
                                            <div className="flex items-baseline gap-1">
                                                <span className="text-[22px] font-black text-[#121117] leading-none">
                                                    ₴{specialist.pricePerLesson}
                                                </span>
                    ...
                    {filtered.length === 0 && (
                        <div className="py-20 text-center">
                            ...
                            <h3 className="text-lg font-semibold text-slate-700 mb-1">
                                {t("catalog.empty_title") || "Нічого не знайдено"}
                            </h3>
                            <p className="text-sm text-slate-400">
                                {t("catalog.empty_desc") || "Спробуйте змінити фільтри або пошуковий запит"}
                            </p>
                        </div>
                    )}
                </div>
