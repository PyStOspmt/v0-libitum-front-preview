                    ...
                    <div className="bg-white rounded-[20px] sm:rounded-[24px] p-6 sm:p-8 border border-slate-200/80 shadow-[0_15px_35px_rgba(0,0,0,0.08)]">
                        ...
                        <div className="space-y-8">
                            {Object.entries(lessonsBySubject).map(([subject, lessons]) => {
                                ...
                                const subjectGrade =
                                    lessons
                                        .filter((l) => l.homework?.grade)
                                        .reduce((acc, l) => acc + (l.homework?.grade || 0), 0) /
                                        ...
                                        <div className="flex items-center justify-between">
                                            ...
                                            <div className="text-right">
                                                <div className="flex items-center gap-1 justify-end">
                                                    ...
                                                    <span className="font-bold text-[18px] text-[#121117]">
                    ...
                    <div className="bg-white rounded-[20px] sm:rounded-[24px] p-6 sm:p-8 border border-slate-200/80 shadow-[0_15px_35px_rgba(0,0,0,0.08)]">
                        ...
                        <Tabs defaultValue="all">
                            <TabsList className="grid w-full max-w-[500px] grid-cols-1 sm:grid-cols-3 rounded-[8px] sm:bg-[#f0f3f3] bg-transparent p-0 sm:p-1 h-auto sm:h-[48px] mb-8 gap-2 sm:gap-0">
                                <TabsTrigger
                                    value="all"
                                    className="rounded-[6px] data-[state=active]:bg-white sm:data-[state=active]:bg-white data-[state=active]:text-[#121117] data-[state=active]:shadow-sm bg-[#f0f3f3] sm:bg-transparent py-2.5 sm:py-1.5 text-[15px] font-[600]"
                                >
...
function LessonPlaceholder({ variant }: { variant: "all" | "completed" | "scheduled" }) {
    const samples: Record<
        string,
        { subject: string; date: string; time: string; status: "completed" | "scheduled"; duration: number; note: string }[]
    > = {
        all: [
            {
            ...
            },
            {
                ...
                status: "completed",
                duration: 60,
                note: "Тригонометрія, домашнє виконано",
            },
        ],
        completed: [
            {
    ...
    return (
        <div className="space-y-4">
            <p className="text-[15px] text-[#69686f]">
                Поки що немає занять у цій вкладці. Ось приклад, як виглядатиме журнал:
            </p>
            {samples[variant].map((item, idx) => (
                <div
                    key={idx}
                    className="rounded-[16px] border border-slate-200/80 bg-white p-6 shadow-[0_4px_12px_rgba(0,0,0,0.03)]"
                >
                    <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                        <div>
                            ...
                            <p className="text-[14px] text-[#69686f] mt-1 flex items-center gap-2">
                                ...
                                {new Date(item.date).toLocaleDateString("uk-UA", { day: "numeric", month: "long" })} •{" "}
                                {item.time}
                                <span className="flex items-center gap-1 ml-2">
                                    <Clock className="h-4 w-4" /> {item.duration} хв
                                </span>
                            </p>
                        </div>
                        <Badge
                            variant="outline"
                            className={`rounded-[6px] px-3 py-1 font-[600] border-0 ${item.status === "completed" ? "bg-[#e8fffb] text-[#00a389]" : "bg-[#f0f3f3] text-[#69686f]"}`}
                        >
...
function LessonCard({ lesson }: { lesson: any }) {
    ...
    return (
        <div
            className={`rounded-[16px] border border-slate-200/80 p-6 shadow-[0_4px_12px_rgba(0,0,0,0.03)] ${statusColors[lesson.status as keyof typeof statusColors]}`}
        >
            <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-4 mb-4">
                ...
                <Badge
                    variant="outline"
                    className={`rounded-[6px] px-3 py-1 font-[600] border-0 ${statusBadgeColors[lesson.status as keyof typeof statusBadgeColors]}`}
                >
                ...
                <div className="space-y-6 mt-6 pt-6 border-t border-gray-100">
                    ...
                    {lesson.report && (
                        <div className="rounded-[12px] bg-[#f0f3f3] p-5">
                            ...
                            <p className="text-[15px] text-[#69686f] leading-relaxed mb-4 bg-white p-4 rounded-[8px] border border-slate-200/50">
                        ...
                        <div className="rounded-[12px] border border-slate-200/80 p-5">
                            ...
                            {lesson.homework.grade && (
                                <div className="mt-4 flex items-start gap-3 rounded-[8px] bg-[#fff8e1] p-4 border border-yellow-100">
                                    ...
                                    <div>
                                        <p className="text-[15px] font-[600] text-[#121117]">
                                            Оцінка: {lesson.homework.grade}/5
                                        </p>
