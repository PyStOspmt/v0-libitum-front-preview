...
const generateLevels = (): Level[] => {
  const levels: Level[] = []
  for (let i = 1; i <= 20; i++) {
    levels.push({ level: i, title: getLevelTitle(i, "junior"), category: "junior", minXP: (i - 1) * 100, maxXP: i * 100, benefits: getJuniorBenefits(i) })
  }
  for (let i = 21; i <= 80; i++) {
    levels.push({ level: i, title: getLevelTitle(i, "pro"), category: "pro", minXP: 2000 + (i - 21) * 150, maxXP: 2000 + (i - 20) * 150, benefits: getProBenefits(i) })
  }
  for (let i = 81; i <= 100; i++) {
    levels.push({ level: i, title: getLevelTitle(i, "expert"), category: "expert", minXP: 11000 + (i - 81) * 200, maxXP: i === 100 ? Number.POSITIVE_INFINITY : 11000 + (i - 80) * 200, benefits: getExpertBenefits(i) })
  }
  return levels
}
...
const availableAchievements: Achievement[] = [
  { id: "first-session", title: "Перше заняття", description: "Завершіть своє перше заняття", icon: "🎯", category: "junior" },
  { id: "first-week", title: "Перший тиждень", description: "Проведіть 5 занять", icon: "📅", category: "junior" },
  { id: "fast-response", title: "Швидка відповідь", description: "Відповідайте на запити менше ніж за 2 години", icon: "⚡", category: "junior" },
  ...
  { id: "master-of-subject", title: "Майстер предмету", description: "Проведіть 200 занять з одного предмету", icon: "📚", category: "pro" },
  ...
  { id: "referral-master", title: "Майстер рефералів", description: "Запросіть 10 активних колег", icon: "🤝", category: "expert" },
  { id: "perfect-score", title: "Ідеальний бал", description: "Середній рейтинг 5.0 з 50+ відгуків", icon: "💎", category: "expert" },
]
...
export const useGamificationStore = create<GamificationStore>()(
  persist(
    (set, get) => ({
      userProgress: {
        "specialist-1": { userId: "specialist-1", level: 45, category: "pro", totalXP: 5750, totalSessions: 48, currentStreak: 5, longestStreak: 12, achievements: [{ ...availableAchievements[0], unlockedAt: "2024-12-01" }, { ...availableAchievements[4], unlockedAt: "2024-12-15" }, { ...availableAchievements[5], unlockedAt: "2025-01-05" }, { ...availableAchievements[7], unlockedAt: "2025-01-20" }], referralXP: 300, ownStudentXP: 450 },
        "client-1": { userId: "client-1", level: 15, category: "junior", totalXP: 1450, totalSessions: 15, currentStreak: 3, longestStreak: 7, achievements: [{ ...availableAchievements[0], unlockedAt: "2024-11-20" }, { ...availableAchievements[4], unlockedAt: "2024-12-10" }], referralXP: 0, ownStudentXP: 0 },
      },

      getLevelInfo: (xp: number) => levels.find((level) => xp >= level.minXP && xp < level.maxXP) || levels[0],

      getProgress: (userId: string) => get().userProgress[userId] || { userId, level: 1, category: "junior", totalXP: 0, totalSessions: 0, currentStreak: 0, longestStreak: 0, achievements: [], referralXP: 0, ownStudentXP: 0 },
      ...
      addSession: (userId: string) => {
        set((state) => {
          ...
          return { userProgress: { ...state.userProgress, [userId]: { ...progress, totalSessions: progress.totalSessions + 1, totalXP: newXP, level: newLevelInfo.level, category: newLevelInfo.category } } }
        })
      },
      ...
      addReferralXP: (userId: string, amount: number) => {
        set((state) => {
          ...
          const newLevelInfo = state.getLevelInfo(newXP)
          return { userProgress: { ...state.userProgress, [userId]: { ...progress, totalXP: newXP, referralXP: progress.referralXP + amount, level: newLevelInfo.level, category: newLevelInfo.category } } }
        })
      },
      ...
      addOwnStudentXP: (userId: string, amount: number) => {
        set((state) => {
          ...
          const newXP = progress.totalXP + amount
          const newLevelInfo = state.getLevelInfo(newXP)
          return { userProgress: { ...state.userProgress, [userId]: { ...progress, totalXP: newXP, ownStudentXP: progress.ownStudentXP + amount, level: newLevelInfo.level, category: newLevelInfo.category } } }
        })
      },
      ...
      unlockAchievement: (userId: string, achievementId: string) => {
        set((state) => {
          ...
          const newLevelInfo = state.getLevelInfo(newXP)
          return { userProgress: { ...state.userProgress, [userId]: { ...progress, achievements: [...progress.achievements, { ...achievement, unlockedAt: new Date().toISOString() }], totalXP: newXP, level: newLevelInfo.level, category: newLevelInfo.category } } }
        })
      },
    }),
