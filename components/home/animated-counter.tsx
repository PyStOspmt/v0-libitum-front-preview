...
function StatCard({ stat, index, visible }: { stat: any; index: number; visible: boolean }) {
    ...
    const count = useCountUp(stat.decimal ? Math.round(stat.value * 10) : stat.value, 2200 + index * 200, visible)
    const display = stat.decimal ? (count / 10).toFixed(1) : count.toLocaleString("uk-UA")
