...
import { CheckCircle, Clock, CreditCard, Headphones, Shield, Zap } from "lucide-react"
        ...
        <div className="relative overflow-hidden py-4 border-y border-slate-100 bg-slate-50/50">
            ...
            <div className="flex animate-marquee whitespace-nowrap">
                {doubled.map((item, i) => {
                    ...
                    return (
                        <div key={i} className="inline-flex items-center gap-2 mx-6 sm:mx-8 flex-shrink-0">
                            <Icon className="h-3.5 w-3.5 text-teal-600 flex-shrink-0" />
                            <span className="text-xs font-medium text-slate-600 tracking-tight">{item.text}</span>
                        </div>
                    )
                })}
            </div>
