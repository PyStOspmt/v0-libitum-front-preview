                                    ...
                                    >
                                        {sessionsInSlot.map((session) => {
                                            ...
                                            return (
                                                ...
                                                >
                                                    <div
                                                        className={`text-[12px] font-[700] ${textColor} hidden sm:block mb-0.5`}
                                                    >
                                                        {session.time}
                                                    </div>
                                                    <div
                                                        className={`truncate text-[11px] sm:text-[12px] font-[600] ${textColor}`}
                                                    >
                                                        {userType === "client" ? session.specialistName : session.clientName}
                                                    </div>
                                                    <div
                                                        className={`truncate text-[11px] sm:text-[11px] font-[500] ${subtitleColor}`}
                                                    >
                                                        {session.subject}
                                                    </div>
                                                    <div className="mt-1 flex items-center justify-between gap-1 text-[11px] sm:text-[11px] text-[#69686f]">
                                                        <div className="flex items-center gap-1 truncate">
                                                            <span
                                                                className={`inline-flex items-center rounded-full px-2 py-0.5 border text-[10px] font-[600] ${session.format === "online" ? "bg-[#e8fffb] border-[#00c5a6]/20 text-[#00a389]" : "bg-blue-50 border-blue-200 text-blue-700"}`}
                                                            >
                                                                {session.format === "online" ? "Онлайн" : "Офлайн"}
                                                            </span>
                                                            <span className="truncate font-[500]">
                                                                {session.price ? `${session.price} грн` : ""}
                                                            </span>
                                                        </div>
                                                        <span className="font-[600] text-[11px] text-[#121117] hover:text-[#00c5a6] transition-colors">
                                                            Деталі
                                                        </span>
                                                    </div>
                                                </div>
                                        ...
                                        {eventsInSlot.map((event, idx) => (
                                            ...
                                            >
                                                <div className="text-[12px] font-[700] hidden sm:block mb-0.5">
                                                    {event.time}
                                                </div>
                                                <div className="truncate text-[11px] sm:text-[12px] font-[600]">
                                                    {event.title}
                                                </div>
                                                <div className="truncate text-[10px] sm:text-[11px] font-[500] opacity-80">
