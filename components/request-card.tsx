        ...
        <CardContent className="p-4 sm:p-6">
              ...
              <div className="flex-1 min-w-0">
                ...
                {userType === "specialist" && request.status !== "pending" && (
                  <div className="flex flex-wrap gap-2 mb-3 sm:mb-4">
                    {request.clientPhone && (
                      <div className="flex items-center gap-1.5 px-2.5 sm:px-3 py-1.5 rounded-[10px] bg-slate-50 border border-slate-100 text-[#4d4c53] font-medium text-[12px] sm:text-[13px]">
                        <Phone className="h-3.5 w-3.5 text-slate-400 shrink-0" />
                        {request.clientPhone}
                      </div>
                    )}
                    {request.clientTelegram && (
                      <div className="flex items-center gap-1.5 px-2.5 sm:px-3 py-1.5 rounded-[10px] bg-[#e8fffb] border border-[#00c5a6]/20 text-[#00a389] font-medium text-[12px] sm:text-[13px]">
                        <MessageCircle className="h-3.5 w-3.5 text-[#00c5a6] shrink-0" />
                        {request.clientTelegram}
                      </div>
                    )}
                  </div>
          ...
          <div className="mt-5 sm:mt-6 pt-4 sm:pt-5 border-t border-slate-100 flex flex-col sm:flex-row gap-2.5 sm:gap-3">
            ...
            {(request.status === "accepted" || request.status === "communicating") && userType === "specialist" && (
              <Button size="sm" onClick={() => setShowCommunicationDialog(true)} className="w-full h-[44px] rounded-[10px] sm:rounded-[12px] bg-[#121117] text-white hover:bg-slate-800 transition-colors">
